<ol class="breadcrumb">
	...
	<li><a href="index.php?page=campagnesClient">Suivi des campagnes</a></li>
...
<?php
	...
	if(isset($modification))
	{
		...
		$messageLog = "Modification de l'action interne <strong>".htmlentities(utf8_decode($partenaireTmp['nom']))."</strong> sur la campagne <strong>".htmlentities($campagneTmp['nom'])."</strong> : <br />";
		$messageLogDetail = "";
		...
		mysqli_query($bdd, 'UPDATE action SET
			...
			codeCampagne = "'.utf8_decode($codeCampagne).'",
			dateDebut = "'.$dateDebut.'",
			dateFin = "'.$dateFin.'",
			objectif = "'.utf8_decode($objectif).'",
			leadsAjustes = "'.utf8_decode($leadsAjustes).'",
			nouveauxProspects = "'.utf8_decode($nouveauxProspects).'",
			tauxOuverture = "'.utf8_decode($tauxOuverture).'",
			tauxClic = "'.utf8_decode($tauxClic).'",
	...
	{
		mysqli_query($bdd, 'INSERT INTO action (idCampagne,idPartenaire,interne,canal,codeCampagne,dateDebut,dateFin,leadsAjustes,nouveauxProspects,tauxOuverture,tauxClic,commentaires, dateModif, userModif) VALUES (
			"'.utf8_decode($id).'",
			"'.utf8_decode($partenaire).'",
			1,
			"'.utf8_decode($canal).'",
			"'.utf8_decode($codeCampagne).'",
			"'.utf8_decode($dateDebut).'",
			"'.utf8_decode($dateFin).'",
			"'.utf8_decode($leadsAjustes).'",
			"'.utf8_decode($nouveauxProspects).'",
			"'.utf8_decode($tauxOuverture).'",
			"'.utf8_decode($tauxClic).'",
		...
		logAction($bdd, $utilisateur['id'], "Ajout d'une action interne pour la campagne <strong>".$campagne['nom']."</strong>: <em>".$partenaire['nom']."</em> (".$codeCampagne.", objectif : ".$objectif.", budget: ".str_replace(",", ".", $budget)."&euro;)", "A");
	...
	else if (isset($suppression))
	{
		...
		echo "<div class='alert alert-success'><button type='button' class='close' data-dismiss='alert'>&times;</button><strong>OK</strong> L'action interne a bien été supprimé.</div>";
	}
	else if (isset($rafraichir))
	{
		...
		while($action = mysqli_fetch_assoc($actions))
		{
			if($action['codeCampagne'] != '')
			{
				...
				$nbLeadsMob = mysqli_fetch_assoc(mysqli_query($bdd, "SELECT COUNT(DISTINCT LEADS_Email) AS leads FROM lead WHERE LEADS_Campagne ='". str_replace(" LP ", " MOB ", $action['codeCampagne']) ."' ".$reqEmail." AND ".$reqTest));
	...
	$clientInfo = mysqli_fetch_assoc(mysqli_query($bdd, 'SELECT * FROM client WHERE id = '.$profilRef));
...
<div class="page-header">
	...
	<a class="btn btn-primary col-sm-offset-10 text-left " href="index.php?page=campagnesClient&filtre=1&submit=1"><span class="glyphicon glyphicon-chevron-left"></span>Retour</a>
</div>

<small>Les leads affichés sur cette page sont mis à jour toutes les <strong>30</strong> minutes. Pour actualiser manuellement les leads, cliquez sur le le bouton <a class="btn btn-default btn-xs" href="index.php?page=campagneClient&id=<?= $campagne['id'] ?>&rafraichir"><span class="glyphicon glyphicon-refresh"></span> Rafraichir</a></small>

<div class="btn-toolbar">
	<div class="btn-group">
		<a class="btn btn-default" href="index.php?page=campagneClient&id=<?= $campagne['id'] ?>&rafraichir"><span class="glyphicon glyphicon-refresh"></span> Rafraichir</a>
		...
		<? if(($campagne['interne'] == 1 || $campagne['interne'] == 2) && $campagne['facture'] == 0) { ?>
			<a class="btn btn-default" href="index.php?page=formCampagneClient&modification&id=<?= $campagne['id'] ?>" <?= ($campagne['facture'] > 1)?('disabled="disabled"'):('') ?>><span class="glyphicon glyphicon-edit"></span> Modifier la campagne</a>
		<? } ?>
		<? if($campagne['interne'] == 1) { ?>
			<a class="btn btn-default" href="index.php?page=supprCampagneClient&id=<?= $campagne['id'] ?>" <?= ($campagne['facture'] > 1)?('disabled="disabled"'):('') ?>><span class="glyphicon glyphicon-remove"></span> Supprimer la campagne</a>
		<? } ?>
	</div>
</div>
...
<div class="panel panel-default">
	...
	<div class="panel-body">
		...
		<dl class="dl-horizontal">
			...
			<dt>Descriptif</dt>
			<dd><?= ($campagne['descriptif'] != "")?(utf8_encode($campagne['descriptif'])):('-') ?></dd>
			...
			<dt>Région</dt>
			<dd><?= utf8_encode(($campagne['regionBI'])?($region[$campagne['regionBI']]):('-')) ?></dd>
...
<div class="panel panel-default">
	...
	<div class="panel-body" style="<?=(!isset($filtreDateDebut) && !isset($filtrePeriode))?('display: none;'):('')?>" id="periodes">
		...
		<div class="row">
			<form id="filtre" method="POST" class="form-inline" action="index.php?page=campagneClient&id=<?= $campagne['id'] ?>&filtre=1">
				...
				<div class="form-group col-xs-3">
					<div class="btn-group">
						...
						<a href="index.php?page=campagneClient&id=<?=$id?>" class="btn btn-default"><span class="glyphicon glyphicon-refresh"></span> Reset</a>
					</div>
				</div>
			</form>
		</div>
		...
		<div class="row">
			<form id="filtre" method="POST" action="index.php?page=campagneClient&id=<?= $campagne['id'] ?>&periode">
				<div class="form-group col-xs-3">
					<div class="input-group date">
						<span class="input-group-addon"><strong>Du</strong></span>
		...
		<div class="well">
			<div class="row">
				<? if (isset($filtreDateDebut) || isset($filtrePeriode)) { ?>
					<a href="index.php?page=campagneClient&id=<?=$id?>" class="btn btn-default btn-sm"><span class="glyphicon glyphicon-remove-circle"></span> Reset</a>
				<?	}
				...
				?>
					<div class="btn-group">
						<a href="index.php?page=campagneClient&id=<?= $campagne['id'] ?>&filtrePeriode=<?=$periode['id']?>" class="btn btn-default btn-sm <?=($filtrePeriode==$periode['id'])?('active'):('')?> text-left" data-toggle="tooltip" data-placement="top" title="<?=$periode['commentaire']?>">
						...
						<a class="btn btn-default btn-sm" href="index.php?page=campagneClient&id=<?= $campagne['id'] ?>&supprimerPeriode=<?=$periode['id']?>">&times;</a>
					</div>
				<? } ?>
			</div>
		</div>
...
<table class="table table-bordered table-hover table-condensed table-striped special">
	<thead>
		<tr>
			...
			<th colspan="9" style="text-align: center;" data-sorter="false">Leads</th>
	...
	<tbody>
		...
		<?php
			...
			while($action = mysqli_fetch_assoc($actions)) { $k++;
				...
				if($action['codeCampagne'] != '' && (isset($filtreDateDebut) || isset($filtrePeriode)))
				{
					...
					$nbLeadsMob = mysqli_fetch_assoc(mysqli_query($bdd, "SELECT COUNT(DISTINCT LEADS_Email) AS leads FROM lead WHERE LEADS_Campagne ='". str_replace(" LP ", " MOB ", $action['codeCampagne']) ."' ".$reqEmail." AND ".$reqTest.$reqFiltreLeads));
		...
		<tr>
			...
			<td><?= number_format($action['budget'], 2, ",", " ") ?> &euro;</td>
			<td>
				<div class="btn-group">
					...
					<ul class="dropdown-menu text-left">
						<li><a href="index.php?page=actionClient&id=<?= $action['id'] ?>"><span class="glyphicon glyphicon-eye-open"></span> Afficher détail</a></li>
						<? if ($campagne['termine'] == 0 && $campagne['facture'] == 0) { ?>
							<li><a href="index.php?page=formActionClient&modification&id=<?= $campagne['id'] ?>&idAction=<?= $action['id'] ?>"><span class="glyphicon glyphicon-edit"></span> Modifier</a></li>
						<? } ?>
					</ul>
				</div>
			</td>
...
<div class="btn-group">
	<a class="btn btn-primary" href="index.php?page=formActionInterneClient&id=<?= $campagne['id'] ?>" ><span class="glyphicon glyphicon-plus"></span> Ajouter une action interne</a>
</div>
...
<table class="table table-bordered table-hover table-condensed table-striped special">
	<thead>
		<tr>
			...
			<th rowspan="2">Fin</th>
			<th colspan="6" style="text-align: center;">Leads</th>
			<th rowspan="2"></th>
		</tr>
		<tr>
			<th title="Web"><span class="glyphicon glyphicon-globe"></span></th>
			<th title="Mobile"><span class="glyphicon glyphicon-phone"></span></th>
			<th title="Tests"><span class="glyphicon glyphicon-thumbs-up"></span></th>
			<th>Total</th>
			<th title="Leads ajustes"><span class="glyphicon glyphicon-saved"></span></th>
			<th title="Nouveaux prospects"><span class="glyphicon glyphicon-user"></span></th>
		</tr>
	</thead>
	<tbody>
		<?php $k = 0; while($action = mysqli_fetch_assoc($actionsInternes)) { $k++; ?>
		...
		<tr>
			...
			<td>
				<div class="btn-group">
					...
					<ul class="dropdown-menu text-left">
						<li><a href="index.php?page=actionInterneClient&id=<?= $action['id'] ?>"><span class="glyphicon glyphicon-eye-open"></span> Afficher détail</a></li>
						<li><a href="index.php?page=formActionInterneClient&modification&id=<?= $campagne['id'] ?>&idAction=<?= $action['id'] ?>" <?= ($campagne['termine'] == 1 || $campagne['facture'] > 1)?('disabled="disabled"'):('') ?>><span class="glyphicon glyphicon-edit"></span> Modifier</a></li>
						<li><a href="index.php?page=supprActionInterneClient&id=<?= $action['id'] ?>" <?= ($campagne['termine'] == 1 || $campagne['facture'] > 1)?('disabled="disabled"'):('') ?>><span class="glyphicon glyphicon-remove"></span> Supprimer</a></li>
					</ul>
				</div>
			</td>
		</tr>
